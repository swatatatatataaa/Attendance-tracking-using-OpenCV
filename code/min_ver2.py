...
#     for person in encode_faces('C:/Users/rawat/OneDrive/Desktop/minor_2/people/'):
...
# Dictionary to store unique labels for each person
person_labels = {}
...
def find_target_face():
    ...
    for person in encode_faces('C:/Users/rawat/OneDrive/Desktop/minor_2/people1/'):
        ...
        label = person_labels.get(filename)
        if label is None:
            label = len(person_labels) + 1  # Assign a new unique label
            person_labels[filename] = label
    ...
    for person in encode_faces('C:/Users/rawat/OneDrive/Desktop/minor_2/people1/'):
        ...
        if face_location:
            ...
            for location in face_location:
                if face_number < len(is_target_face):  # Ensure face_number is within the bounds of is_target_face list
                    if is_target_face[face_number]:
                        create_frame(location, str(label))  # Ensure label is converted to string
                        makeAttendanceEntry(label, True)
                    else:
                        makeAttendanceEntry(label, False)
                else:
